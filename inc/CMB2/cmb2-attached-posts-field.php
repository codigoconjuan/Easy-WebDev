...
class WDS_CMB2_Attached_Posts_Field {
	...
	public function render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		foreach ( $posts as $post ) {
			...
			// Build our list item
			echo '<li data-id="', $post->ID ,'" class="' . $zebra . $added . '">', $thumbnail ,'<a title="'. __( 'Edit' ) .'" href="', get_edit_post_link( $post ) ,'">', get_the_title( $post ) . '<strong> Post Type:</strong> <em>  ' . get_post_type($post) . '</em>' . '</a><span class="dashicons dashicons-plus add-remove"></span></li>';
